...
MuzPlyModel::MuzPlyModel(Ply ply)
{
    this->ply_ = ply;
}
...
bool MuzPlyModel::from_string(MuzTurnModel turn, std::string_view half_ply_str)
{
	// 空 or 空白だけ → 即失敗
	if (half_ply_str.empty()) return false;
	...
	// → 両方チェックするのがベストプラクティス
	if (ec != std::errc{} || ptr != half_ply_str.data() + half_ply_str.size()) return -1;
	...
	// 
	// 対局を記録する方式で、チェスでは人間もコンピューターも FEN を使い、将棋では人間もコンピューターもまちまちだが、Stockfish 系の将棋の思考エンジンは SFEN を使っている。
    // FEN では ply を記録し、 SFEN では half-ply を記録する。
	// 
	// しかし、開始局面で 1 から始まって記録されているのは、コンピューターを使って式を立てるにはめんどくさいので、人間の常識を無視して 0 から始めたい（＾～＾）
	...
	this->ply_ = std::max(2 * (value - 1), 0) + static_cast<int>(turn.is_white());
	return true;
}
